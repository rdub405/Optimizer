...
#rotonba<-rotonba[!(rotonba$Id==70),]
###############################################Player Ranking##########################################################
rotonba <- transform(rotonba, 
                     PosRank = ave(Points, Position, 
...
rotonba <- sqldf('select a.*
                  , b.Time, b.Opponent, b.Line, b.OverUnder, b.ProjectedPTS
                 from rotonba a
...
find_teams <- function(train, cap, constraint = c("none", "all_diff", "no_opp"), 
  ...
  colnames(train) <- c( "Id", "Name", "Salary", "Team", "Position", "Oppt", "Ceil", "Floor", "Points","Value","PosRank", "Time", "Opponent", "Line", "OverUnder", "ProjectedPTS")
...
write.csv(lineups, file = "C:/Users/williarl/Documents/Machine Learning/main_nba_lineup.csv")

expo <- sqldf('select Name, Position, count(Name) as "NameCount"
              from lineups
...
expo
write.csv(expo, file = "C:/Users/williarl/Documents/ShinyOut/expo.csv")
