...
###############################Money Line#######################################
setwd("C:/Users/williams/Documents/NBA/NBA")
...
nba.odds <- html_nodes(webpage,'#tschedules')
tabledata <- html_table(nba.odds[[1]],fill = TRUE, header = TRUE)
tabledata$Over_Under <- tabledata$`Over/Under`
tabledata$Projected_Points <- tabledata$`Projected Points`
tabledata$`Over/Under` <- NULL
tabledata$`Projected Points` <- NULL
tabledata$Moneyline <- NULL
#############################################################################################################


library(sqldf)
express <- sqldf('select a.*,b.Time, b.Line, b.Over_Under, b.Projected_Points 
                 from rotonba a
                 left join tabledata b on a.Team = b.Team')
...
find_teams <- function(train, cap, constraint = c("none", "all_diff", "no_opp"), 
  ...
  colnames(train) <- c( "Id", "Name", "Salary", "Team", "Position", "Oppt", "Ceil", "Floor", "Points","Value","PosRank")
  ...
  set.objfn(lpfantasy, train$Points)
...
#write.csv(rotonba, file = "C:/Users/williams/Documents/NBA/main_nba_lineup.csv", fileEncoding = "UTF-16LE")
