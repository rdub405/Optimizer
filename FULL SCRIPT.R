...
jump <- seq(0, 200, by = 100)
url_log <- paste('https://www.basketball-reference.com/play-index/pgl_finder.cgi?request=1&player_id=&match=game&year_min=2017&year_max=2017&age_min=0&age_max=99&team_id=&opp_id=&season_start=1&season_end=-1&is_playoffs=N&round_id=&game_num_type=&game_num_min=&game_num_max=&game_month=&game_day=&game_location=&game_result=&is_starter=&is_active=&is_hof=&pos_is_g=Y&pos_is_gf=Y&pos_is_f=Y&pos_is_fg=Y&pos_is_fc=Y&pos_is_c=Y&pos_is_cf=Y&c1stat=&c1comp=&c1val=&c2stat=&c2comp=&c2val=&c3stat=&c3comp=&c3val=&c4stat=&c4comp=&c4val=&is_dbl_dbl=&is_trp_dbl=&order_by=player&order_by_asc=&offset=', jump, sep="")

dfList <- lapply(url_log, function(i) {
  html_log <- read_html(i)
  node_log <- html_nodes(html_log, 'table')
  table_log <- html_table(node_log)[[1]]
})

table_log <- do.call(rbind, dfList)             # ASSUMING ALL DFs MAINTAIN SAME COLS
